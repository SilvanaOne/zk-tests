...
//! Simplified for devnet - no localhost resolution needed.
...
/// Creates a simple HTTP client for devnet
pub fn create_client() -> reqwest::Result<reqwest::Client> {
    reqwest::Client::builder()
        .user_agent("advanced-payment-devnet-cli/0.1")
        .build()
}
...
impl ContractBlobsContext {
    pub async fn fetch() -> anyhow::Result<Self> {
        ...
        // Use PARTY_PROVIDER for FeaturedAppRight lookup (provider has the app registration)
        // The provider must have FeaturedAppRight for app rewards in Transfer
        let party_provider = std::env::var("PARTY_PROVIDER").ok();
        let provider_hint = party_provider
            .as_deref()
        ...
        // Extract DSO party from AmuletRules payload
        let dso_party = dso
