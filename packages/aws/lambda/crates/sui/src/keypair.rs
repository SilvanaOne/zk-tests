...
mod tests {
    ...
    #[test]
    fn test_generate_keypair_and_print() {
        let keypair = generate_ed25519().expect("Failed to generate keypair");
        
        println!("Generated Keypair:");
        println!("Secret Key: {:?}", keypair.secret_key);
        println!("Public Key: {:?}", keypair.public_key);
        println!("Address: {}", keypair.address);
        println!("Sui Private Key: {}", keypair.sui_private_key);
        
        // Verify that the address corresponds to the private key
        let reconstructed = from_sui_private_key(&keypair.sui_private_key)
            .expect("Failed to reconstruct keypair from private key");
        
        assert_eq!(keypair.address, reconstructed.address, 
                   "Generated address doesn't match reconstructed address");
        assert_eq!(keypair.secret_key, reconstructed.secret_key,
                   "Secret keys don't match");
        assert_eq!(keypair.public_key, reconstructed.public_key,
