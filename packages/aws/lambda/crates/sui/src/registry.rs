use crate::chain::{get_reference_gas_price, load_sender_from_env, pick_gas_object};
...
pub async fn create_registry(
    ...
    debug!(
        "Creating registry '{}' on chain '{}' with package '{}'",
        name, chain, registry_package
    );
    ...
    // Parse package ID
    let package_id =
        sui::Address::from_str(registry_package).context("Failed to parse registry package ID")?;
    ...
    info!(
        "Transaction executed on blockchain: {} (took {}ms)",
    ...
    let registry_id = if let Some(executed) = tx_resp.transaction.as_ref() {
        if let Some(events) = executed.events.as_ref() {
            ...
            for (idx, e) in events.events.iter().enumerate() {
                debug!(
                    "Event[{}]: type={:?}, module={:?}, json_present={}",
                    idx,
                    e.event_type,
                    e.module,
                    e.json.is_some()
                ...
                executed
                    ...
                    .and_then(|effects| {
                        effects
                            ...
                            .iter()
                            .find(|obj| {
                                let is_created = obj.input_state
                                    == Some(
                                        proto::changed_object::InputObjectState::DoesNotExist
                                            as i32,
                                    );
                                let is_registry = obj
                                    .object_type
                                ...
                                if is_created {
                                    info!(
                                        "Created object: id={:?}, type={:?}, is_registry={}",
...
pub async fn remove_app(
    ...
    execute_registry_function(
        ...
        "remove_app",
        |tb| {
            vec![tb.input(sui_transaction_builder::Serialized(&MoveString {
                bytes: name.into_bytes(),
            }))]
        },
    )
...
async fn fetch_initial_shared_version(chain: &str, object_id: &str) -> Result<u64> {
    ...
    for attempt in 1..=10 {
            ...
            Err(status) => {
                ...
                if attempt < 10 {
                    ...
                    return Err(anyhow::anyhow!(
                        "Failed to fetch object after 10 attempts: {}",
                        status
                    ));
                }
            }
        ...
        if let Some(obj) = resp.object {
            info!(
                "Attempt {} - object found: id={:?} version={:?} owner={:?}",
                    ...
                    if let Ok(kind_enum) = proto::owner::OwnerKind::try_from(kind_i32) {
                        ...
                        if kind_enum == proto::owner::OwnerKind::Shared {
                            if let Some(v) = owner.version {
                                ...
                                info!(
                                    "Shared owner but missing initial_shared_version; retrying..."
                                );
                            }
                        } else {
...
{
    info!(
        "Executing {} on chain '{}' with registry '{}'",
        function_name, chain, registry_id
    );
    ...
    let registry_obj_id =
        sui::Address::from_str(registry_id).context("Failed to parse registry object ID")?;
    let clock_id =
        sui::Address::from_str(SUI_CLOCK_OBJECT_ID).context("Failed to parse clock object ID")?;
    ...
    let initial_shared_version = fetch_initial_shared_version(chain, registry_id)
        .await
        .context("Failed to fetch initial shared version for registry object")?;

    debug!(
        "Registry object initial shared version: {}",
        initial_shared_version
    );

    // Build function arguments
    // Registry is a shared mutable object with the fetched initial version
    let registry_obj = tb.input(sui_transaction_builder::unresolved::Input::shared(
        registry_obj_id,
        initial_shared_version,
        true, // mutable
    ));
    ...
    let clock_obj = tb.input(sui_transaction_builder::unresolved::Input::shared(
        clock_id, 1, false,
    ));
    ...
    let grpc_creation_time = start.elapsed();
    info!(
        "GRPC client creation took in registry: {:?} ns",
        grpc_creation_time.as_nanos()
    );
    let mut exec = grpc.execution_client();
    ...
    info!(
        "[sui::registry] {} executed successfully with tx_digest: {} (took {}ms)",
