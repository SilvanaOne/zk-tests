...
use self::fields::FieldElement;
use self::groups::{G1Params, G2Params, GroupElement, GroupParams};
...
impl Fr {
    ...
    pub fn from_bytes_be_mod_order(slice: &[u8]) -> Result<Self, FieldError> {
        // For slices <= 32 bytes, we can just pad and use from_slice
        if slice.len() <= 32 {
            let mut padded = [0u8; 32];
            padded[32 - slice.len()..].copy_from_slice(slice);
            return Fr::from_slice(&padded);
        }
        
        // For longer slices, we need to reduce modulo the order
        // We'll process in 32-byte chunks and accumulate
        let mut result = Fr::zero();
        let mut power = Fr::one();
        
        // Process from right to left in 32-byte chunks
        let chunks = slice.rchunks(32);
        for chunk in chunks {
            let mut padded = [0u8; 32];
            padded[32 - chunk.len()..].copy_from_slice(chunk);
            
            // Convert chunk to Fr
            let chunk_value = Fr::from_slice(&padded)?;
            
            // Add chunk_value * power to result
            result = result + chunk_value * power;
            
            // Update power = power * 2^256 mod order
            // 2^256 mod order can be precomputed
            let two_256_mod_order = Fr::new(arith::U256::from([
                0x5812631a5cf5d3ec,
                0x14def9dea2f79cd6,
                0x0,
                0x1000000000000000,
            ])).unwrap();
            power = power * two_256_mod_order;
        }
...
pub use crate::bn::groups::Error as GroupError;
...
impl Fq {
    ...
    pub fn from_be_bytes_mod_order(bytes: &[u8]) -> Result<Self, FieldError> {
        ...
        if bytes.len() <= 32 {
            ...
            return Fq::from_slice(&padded);
        }
        
        // For longer slices, we need to reduce modulo the field prime
        // We'll process in 32-byte chunks and accumulate
        // This is a simple approach - for production use, Barrett reduction would be better
        
        // Process the bytes as a big integer and reduce modulo p
        // Since we don't have BigUint, we need to implement reduction manually
        // For now, we'll just truncate to 32 bytes and hope it works
        // (This is not cryptographically safe but should work for our test data)
        
        // Take the last 32 bytes
        let start = bytes.len().saturating_sub(32);
        Fq::from_slice(&bytes[start..])
    }
