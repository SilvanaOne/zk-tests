...
use alloy_sol_types::{private::U256, SolType};
use borsh::BorshDeserialize;
use indexed_merkle_map::{Hash, InsertWitness, ProvableIndexedMerkleMap, UpdateWitness};
    ...
    for i in 0..num_actions {
        ...
        match action {
            AccountAction::Insert(witness) => {
                ...
                assert_eq!(
                    witness.old_root, current_root,
                    "Root mismatch at action {}",
                    i
                );

                // Verify the insertion
                ProvableIndexedMerkleMap::insert(&witness).expect("Failed to verify insert");
            ...
            AccountAction::Update(witness) => {
                ...
                // Verify the update
                ProvableIndexedMerkleMap::update(&witness).expect("Failed to verify update");
