...
) -> Result<(Vec<u8>, Vec<u8>, AttestationDocument), Box<dyn std::error::Error>> {
...
impl CoseSign1 {
    ...
    fn to_signed_message(&self) -> Result<Vec<u8>, NitroAttestationVerifyError> {
        ...
        ciborium::ser::into_writer(&value, &mut bytes).map_err(|_| {
            NitroAttestationVerifyError::InvalidCoseSign1("cannot parse message".to_string())
        })?;
        Ok(bytes)
    }
}
...
fn verify_cert_chain(cert_chain: &[&[u8]], now_ms: u64) -> Result<(), NitroAttestationVerifyError> {
    ...
    for i in 0..cert_chain.len() {
        ...
        if !cert.validity().is_valid_at(now_secs) {
            println!("Certificate timestamp not valid: {:?}", cert);
