...
fn init_console() {
    ...
    for (path, mode, fd) in args {
        match freopen(path, mode, fd) {
            Ok(()) => dmesg(format!("Opened {}", path)),
            Err(e) => eprintln!("{}", e),
        }
    }
}
...
fn boot() {
    ...
    init_console();
    init_platform();
    // Seed entropy into the kernel randomness pool
    match seed_entropy(32, get_entropy) {
        Ok(_) => dmesg("Seeded entropy successfully".to_string()),
        Err(e) => eprintln!("Failed to seed entropy: {}", e),
    }
}
